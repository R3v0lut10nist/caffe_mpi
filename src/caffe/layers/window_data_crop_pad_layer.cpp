...
WindowDataCropPadLayer<Dtype>::~WindowDataCropPadLayer<Dtype>() {
	this->JoinPrefetchThread();
}

template <typename Dtype>
void WindowDataCropPadLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	do {
		...
		for (int i = 0; i < num_windows; ++i) {
			...
			window[WindowDataCropPadLayer::Y1] = y1;
			window[WindowDataCropPadLayer::X2] = x2;
			window[WindowDataCropPadLayer::Y2] = y2;

			// add window to foreground list or background list
			if (overlap >= fg_threshold) {
				int label = window[WindowDataCropPadLayer::LABEL];
				CHECK_GT(label, 0);
				...
				window[WindowDataCropPadLayer::LABEL] = 0;
				window[WindowDataCropPadLayer::OVERLAP] = 0;
...
template <typename Dtype>
unsigned int WindowDataCropPadLayer<Dtype>::PrefetchRand() {
	CHECK(prefetch_rng_);
...
template <typename Dtype>
void WindowDataCropPadLayer<Dtype>::InternalThreadEntry() {
	...
	const string& multi_crop_type = this->layer_param_.window_data_param().multi_crop_type();
	...
	for (int is_fg = 0; is_fg < 2; ++is_fg) {
		for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
			...
			pair<std::string, vector<int> > image =
					image_database_[window[WindowDataCropPadLayer<Dtype>::IMAGE_INDEX]];

			cv::Mat cv_img;
			if (this->cache_images_) {
				pair<std::string, Datum> image_cached =
						image_database_cache_[window[WindowDataCropPadLayer<Dtype>::IMAGE_INDEX]];
			...
			int x1 = window[WindowDataCropPadLayer<Dtype>::X1];
			int y1 = window[WindowDataCropPadLayer<Dtype>::Y1];
			int x2 = window[WindowDataCropPadLayer<Dtype>::X2];
			int y2 = window[WindowDataCropPadLayer<Dtype>::Y2];
			...
			if (multi_crop_type=="up") y2 = y2-(y2-y1+1)/2;
			if (multi_crop_type=="down") y1= y1+(y2-y1+1)/2;
			if (multi_crop_type=="left") x2 = x2-(x2-x1+1)/2;
			if (multi_crop_type=="right") x1 = x1+(x2-x1+1)/2;
...
//			LOG(INFO) << "!!!!!!!!!crop images";
			...
			cv::Mat tmp_cropped_img = cv_img(roi);

			// create padding mean
			cv::Mat tmp_pad_img(tmp_cropped_img.rows, tmp_cropped_img.cols, tmp_cropped_img.type(), 128);
			if (this->has_mean_values_) {
				tmp_pad_img.setTo(cv::Scalar(mean_values_[0], mean_values_[1], mean_values_[2]));
			}
			// concate image with padding img
			cv::Mat cv_cropped_img(tmp_cropped_img.rows*2, tmp_cropped_img.cols*2, tmp_cropped_img.type(), 128);
			// concate cropped patch with image mean
			if (multi_crop_type=="up") cv::vconcat(tmp_cropped_img, tmp_pad_img, cv_cropped_img);
			if (multi_crop_type=="down") cv::vconcat(tmp_pad_img, tmp_cropped_img, cv_cropped_img);
			if (multi_crop_type=="left") cv::hconcat(tmp_cropped_img, tmp_pad_img, cv_cropped_img);
			if (multi_crop_type=="right") cv::hconcat(tmp_pad_img, tmp_cropped_img, cv_cropped_img);
			// resize to crop_size
			...
			// get window label
			top_label[item_id] = window[WindowDataCropPadLayer<Dtype>::LABEL];
			...
			inf << image.first << std::endl
					<< window[WindowDataCropPadLayer<Dtype>::X1]+1 << std::endl
					<< window[WindowDataCropPadLayer<Dtype>::Y1]+1 << std::endl
					<< window[WindowDataCropPadLayer<Dtype>::X2]+1 << std::endl
					<< window[WindowDataCropPadLayer<Dtype>::Y2]+1 << std::endl
					<< do_mirror << std::endl
...
INSTANTIATE_CLASS(WindowDataCropPadLayer);
REGISTER_LAYER_CLASS(WindowDataCropPad);
