 ...
 */
template <typename Dtype>
class WindowDataCropPadLayer : public BasePrefetchingDataLayer<Dtype> {
public:
	explicit WindowDataCropPadLayer(const LayerParameter& param)
	: BasePrefetchingDataLayer<Dtype>(param) {}
	virtual ~WindowDataCropPadLayer();
	virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
			const vector<Blob<Dtype>*>& top);
 ...
 */
template <typename Dtype>
class WindowDataMultiContextLayer : public BasePrefetchingDataLayer<Dtype> {
public:
	explicit WindowDataMultiContextLayer(const LayerParameter& param)
	: BasePrefetchingDataLayer<Dtype>(param) {}
	virtual ~WindowDataMultiContextLayer();
	virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
			const vector<Blob<Dtype>*>& top);

	virtual inline const char* type() const { return "WindowDataMultiContext"; }
	virtual inline int ExactNumBottomBlobs() const { return 0; }
	...
	int total_windows_num() { return total_windows_.size(); }

	int set_total_windows_idx(int total_windows_idx){ 
	    this->total_windows_idx_ =  total_windows_idx;
	    return this->total_windows_idx_;
	}
...
protected:
	...
	vector<vector<float> > fg_windows_;
	vector<vector<float> > bg_windows_;
	vector<vector<float> > fg_windows_eff_labels_;
	vector<vector<float> > bg_windows_eff_labels_;
	vector<vector<float> > fg_windows_region_eff_labels_;
	vector<vector<float> > bg_windows_region_eff_labels_;

  	vector<vector<float> > total_windows_;
  	vector<vector<float> > total_windows_eff_labels_;
  	vector<vector<float> > total_windows_region_eff_labels_;
	...
	bool cache_images_;
	vector<std::pair<std::string, Datum > > image_database_cache_;
	Blob<Dtype> prefetch_region_label_;
  	vector<Dtype> multi_context_pad_;
  	vector<Dtype> multi_rotation_degree_;
};
